...
class TestValidateFieldUserStatusCodeAPI(BaseTestMixin, BaseTestUserContext):
    ...
    def test_get_follower_list_non_user(self, client):
        response = client.get(resolve_url('users:follower', user_id='non'))
    ...
    def test_get_follower_list_deleted_user(self, client):
        ...
        response = client.get(resolve_url('users:follower', user_id='deleted_user'))

        assert response.status_code == 404

    def test_get_follower_list_by_anonymous_user(self, client):
        response = client.get(resolve_url('users:me-follower'))
